...
// Update schema in localStorage if needed (Flash version)
const updateLocalStorageSchema = () => {
  ...
  createLocalStorageDispatch('flash', (data) => {
    v2Converter(data);
    return data;
  })();
};
...
export default function App() {
  ...
  const isRegistered = useLocalStorageSelector('flash', (state) => !!state);
  ...
  // Notify service worker that Flash is ready
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  // Load WebTorrent and other required scripts once user is registered
  useEffect(() => {
  ...
  // Redirect to main page if offline
  useEffect(() => {
