...
const TORRENT_SIZE_LIMIT = 10 * 1024 * 1024 * 1024; // 4 GB
const WS_SIZE_LIMIT = 700 * 1024 * 1024; // 700 MB
...
class FileTransfer extends PureComponent {
  ...
  constructor(props) {
    ...
    const savedData = JSON.parse(localStorage.getItem('flash'));
    this.client = {
      ...savedData.user,
  ...
  componentDidMount() {
    document.title = `${this.client.room} room | Flash`;
  ...
  handleShare = () => {
    ...
    navigator.share({
      title: 'Share files',
      text: `Join my room '${this.client.room}' on Flash to share files`,
